...
1.  **Salve o Script**: Salve o conteúdo do script `fakeroot_permissions.sh` em um arquivo, por exemplo, `/home/$USER/fakeroot_permissions.sh`.

2.  **Torne o Script Executável**: Abra um terminal e execute o seguinte comando para dar permissão de execução ao script:
    ```bash
    chmod +x /home/$USER/fakeroot_permissions.sh
    ```

3.  **Execute o Script**: Execute o script a partir do terminal:
    ```bash
    ./home/$USER/fakeroot_permissions.sh
    ```
